...
class UpdateTemplate extends Component {
  constructor(props) {
    ...
    this.state = {
      TempleteName:"",
      isLoading: false,
      filter_items: [{ "unique_key": v4(), "MatricsName": "", "MatricsId": "", "FilterTypeId": "", "FilterValue": "" }],
      matric_items: [{ "unique_key": v4(), "Matric": 1, "Weight": "", "Method": "" }],
      Catagory_items: [{ "unique_key": v4(), "CategoryName": "", "Weight": "", "Matric": [] }],
      selectedItem: "",
  ...
  handleAddcatagoryItem = () => {
    this.setState({
      Catagory_items: ([...this.state.Catagory_items, { "unique_key": v4(), "CategoryName": "", "Weight": "", "Matric": [] }])
    });
  };
  ...
  handleCatagoryItemChange = (idx, evt) => {
    ...
    this.setState({ Catagory_items: newCategoryItems });
  };
  ...
  handleAddFilterItem = () => {
    this.setState({
      filter_items: ([...this.state.filter_items, { "unique_key": v4(), "MatricsName": "", "MatricsId": "", "FilterTypeId": "", "FilterValue": "" }])
    });
  };
  ...
  renderfilter() {
    return this.state.filter_items.map((item, idx) =>
      <div key={item.unique_key} className="row">
        ...
        <div className="col-sm-2">
          <label class="Label-text col-sm-12 ml-1">Filter Type</label>
  ...
  handleAddMatricItem = (categoryID) => {
    const newCategoryItems = this.state.Catagory_items.map((item, sidx) => {
      if (categoryID !== sidx) return item;
        ...
        item.Matric = [...item.Matric, { "unique_key": v4(), "MatricsName": "", "MatricsId": "", "Weight": "", "Method": "" }]
      return item;
    });
    this.setState({
  ...
  handleChangeMatricItem = (categoryIndex, matricIndex, evt) => {
    const newCategoryItems = this.state.Catagory_items.map((item, sidx) => {
      ...
      const Matric = item.Matric.map((item, sidx) => {
        if (matricIndex !== sidx) return item;
        return { ...item, [evt.target.name]: evt.target.value };
      });
  ...
  rendermatric(matricList, categoryIndex) {
    ...
    return matricList.map((item, idx) =>
      <div key={item.unique_key} className="row">
        ...
        <div className="col-sm-2">
          <label class="Label-text col-sm-12 ml-2">Method</label>
          <div class="form-group">
            ...
            <div className="col-md-12 p-0">
              <div className="inventry-wrapper m-2 ">
                <div className="row">
                  ...
                  <input className="SHOP-DETAILS input-text pl-3" value={this.state.TempleteName}  name="TempleteName" onChange={(x) => this.setState({"TempleteName":x.target.value})}/>
                  </div>
