...
class TemplateDetail extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.location.state != null && this.props.location.state.TempleteName) {
      this.setState({ TempleteName: this.props.location.state.TempleteName ,
    ...
    if (this.props != null && this.props.location.state != null && this.props.location.state.index) {
      ...
      const { index, admin, TempleteName } = this.props.location.state;
      if (admin === false && !TempleteName)
        this.setState({ TempleteId: index.TempleteId, TempleteName: index.TempleteName })
      else {
        this.setState({ TempleteName: TempleteName })
      }
      var filter_items = [];
      console.log(index)
      ...
      for (var i = 0; i < index.indexCategory.length; i++) {
        ...
        var matric_list = []
        total = total + index.indexCategory[i].Weight;
        var matricWeightTotal = 0
        for (var j = 0; j < index.indexCategory[i].indexcategory_matric.length; j++) {
          matricWeightTotal = matricWeightTotal + index.indexCategory[i].indexcategory_matric[j].Weight
        ...
        category_items = [...category_items, {
          ...
          "Matric": matric_list,
          "total":matricWeightTotal
        }]
      }
      this.setState({ filter_items: filter_items })
      this.setState({ Catagory_items: category_items ,totalWeight:total })
  ...
  addIndex = () => {
    console.log(this.state.filter_items)
    console.log(this.state.Catagory_items)
    const data = {
    ...
    console.log(data);
    if (this.state.TempleteId) {
      this.props.updateTemplate(data).then((res) => {
        if (res.status && res.status == "success") {
          new Noty({
            text: "Succsessfully Updated Index",
            layout: "topRight",
            theme: "bootstrap-v4",
            type: "success",
            timeout: 1000
          }).show();
          this.props.history.push("/template/allgrid")
      ...
      this.props.addTemplate(data).then((res) => {
        if (res.status && res.status == "success") {
          new Noty({
            text: "Succsessfully Created Index",
            layout: "topRight",
            theme: "bootstrap-v4",
            type: "success",
            timeout: 1000
          ...
          <div className="row col-md-12 Container pl-0 pr-0">
            ...
            <div className="col-md-12 p-0">
              ...
              <div className="inventry-wrapper m-2 ">
                ...
                <div className="text-right mt-2 mr-5">
                  <button class="submit-btn2 text-center" onClick={() => this.addIndex()}>&nbsp; &nbsp; Submit &nbsp; &nbsp;</button>
                </div>
              </div>
            </div>
          </div>
